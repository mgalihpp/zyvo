 ...
 *
 * The initial panel is adopted from the URL on the server (see the builder
 * page) and seeded into the store during hydration, so there is no client-side
 * adoption here — that avoids the "personal -> URL panel" first-paint flicker.
 * This hook only writes subsequent store changes back to the URL with
