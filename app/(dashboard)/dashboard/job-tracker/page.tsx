...
import { ClockIcon, TargetIcon, UsersIcon, BellIcon, PlusIcon, ArrowRightIcon, CheckCircleIcon, AlertCircleIcon } from "lucide-react";
    ...
    >
      <defs>
        ...
        <filter id="softShadow" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="12" stdDeviation="20" floodColor="var(--primary)" floodOpacity="0.15" />
        </filter>
      </defs>

      {/* Kanban board background */}
      <rect x="20" y="20" width="440" height="300" rx="16" fill="url(#boardGrad)" filter="url(#softShadow)" />

      {/* Column headers */}
      <g fontFamily="system-ui, -apple-system, sans-serif" fontSize="12" fontWeight="600" fill="currentColor" fillOpacity="0.45">
        <text x="58" y="54" textAnchor="middle">DILAMAR</text>
        <text x="178" y="54" textAnchor="middle">INTERVIEW</text>
        <text x="302" y="54" textAnchor="middle">OFFER</text>
        <text x="422" y="54" textAnchor="middle">DITERIMA</text>
      </g>

      {/* Column separators */}
      <line x1="118" y1="36" x2="118" y2="304" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" strokeDasharray="4 4" />
      <line x1="238" y1="36" x2="238" y2="304" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" strokeDasharray="4 4" />
      <line x1="358" y1="36" x2="358" y2="304" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" strokeDasharray="4 4" />

      {/* Cards in Applied column */}
      <rect x="38" y="68" width="144" height="68" rx="10" fill="url(#cardGrad1)" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" />
      <rect x="38" y="148" width="144" height="68" rx="10" fill="url(#cardGrad1)" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" />
      <rect x="38" y="228" width="144" height="52" rx="10" fill="url(#cardGrad1)" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" fillOpacity="0.6" />

      {/* Cards in Interview column */}
      <rect x="158" y="68" width="144" height="68" rx="10" fill="url(#cardGrad2)" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" />
      <rect x="158" y="148" width="144" height="68" rx="10" fill="url(#cardGrad2)" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" />

      {/* Cards in Offer column */}
      <rect x="278" y="68" width="144" height="68" rx="10" fill="url(#cardGrad3)" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" />

      {/* Cards in Hired column */}
      <rect x="398" y="68" width="144" height="68" rx="10" fill="url(#cardGrad4)" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" />
      <rect x="398" y="148" width="144" height="68" rx="10" fill="url(#cardGrad4)" stroke="currentColor" strokeOpacity="0.12" strokeWidth="1.5" />
      ...
      <g opacity="0.6">
        <circle cx="450" cy="260" r="8" fill="var(--primary)" fillOpacity="0.15" />
        <circle cx="465" cy="245" r="5" fill="var(--chart-2)" fillOpacity="0.15" />
        <circle cx="435" cy="248" r="4" fill="var(--chart-5)" fillOpacity="0.15" />
      </g>
...
function FeatureCard({ icon: Icon, title, description, delay = 0 }: {
  ...
  return (
    <Card className="relative overflow-hidden border-border/50 bg-card/80 backdrop-blur-sm" style={{ animationDelay: `${delay}ms` }}>
...
function WaitlistForm() {
    ...
    return (
      <div className="text-center py-8">
        <div className="mx-auto mb-4 flex size-16 items-center justify-center rounded-full bg-green-100 dark:bg-green-900/30">
          <CheckCircleIcon className="size-8 text-green-600 dark:text-green-400" aria-hidden="true" />
        </div>
        <h3 className="text-lg font-semibold">Kamu sudah terdaftar!</h3>
        <p className="mt-1 text-sm text-muted-foreground">Periksa email {email} untuk konfirmasi.</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2 max-w-md mx-auto">
      ...
      />
      <Button type="submit" disabled={loading || !email} className="whitespace-nowrap" aria-label="Daftar notifikasi">
        {loading ? (
          <>
    ...
    <div className="mx-auto flex max-w-5xl flex-col items-center gap-10 px-4 py-12 sm:py-20 lg:py-24">
      ...
      <div className="relative w-full max-w-4xl aspect-[480/340]">
        ...
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-[600px] rounded-full bg-gradient-to-tr from-primary/10 via-transparent to-chart-2/10 blur-3xl animate-pulse" aria-hidden="true" />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 text-sm font-medium text-primary">
        <span className="relative flex h-2 w-2">
          <span className="absolute inset-0 animate-ping rounded-full bg-primary/75" aria-hidden="true" />
          <span className="relative rounded-full bg-primary" aria-hidden="true" />
        </span>
      ...
      <div className="text-center space-y-4">
        ...
        <p className="mx-auto max-w-2xl text-lg text-muted-foreground">
          Kelola semua lamaran pekerjaanmu di satu tempat. Kanban board visual, statistik
          konversi, reminder follow-up, dan sinkronisasi dengan CV Builder — semua terintegrasi.
        </p>
      </div>
      ...
      <div className="w-full max-w-xl text-center space-y-6">
        ...
        <p className="text-sm text-muted-foreground">
          Fitur ini akan tersedia untuk paket <strong className="text-foreground">Basic</strong> dan{" "}
          <strong className="text-primary">Pro</strong>. Gratis: 1 CV + unduhan PDF tak terbatas.
        </p>
      ...
      <div className="w-full max-w-2xl rounded-xl border bg-card p-6 shadow-sm">
        ...
        <ol className="space-y-4">
          {[
            { phase: "Fase 1", title: "MVP Pipeline", desc: "Board Kanban, CRUD lamaran, status drag-drop", badge: "Q4 2024" },
            { phase: "Fase 2", title: "Notifikasi & Analitik", desc: "Email reminder, statistik konversi, ekspor CSV", badge: "Q1 2025" },
            { phase: "Fase 3", title: "Integrasi Penuh", desc: "Sinkron CV Builder, template surat lamaran, AI follow-up", badge: "Q2 2025" },
          ].map((item, i) => (
            <li key={i} className="relative pl-8 pb-4 last:pb-0 before:absolute before:left-0 before:top-1 before:h-full before:w-0.5 before:bg-border before:last:hidden">
              ...
              <div className="ms-3">
                <div className="flex items-baseline gap-2">
                  <span className="text-xs font-semibold text-primary uppercase tracking-wide">{item.phase}</span>
